...
function applyReferralCommission(purchaserUser, planAmount) {
  ...
  if (supabase) {
    supabase.from('users').update({ wallet_balance: referrer.walletBalance }).eq('id', referrer.id)
...
app.post('/api/users/request-withdrawal', async (req, res) => {
  ...
  const alreadyPending = pendingWithdrawals.find(w => w.userId === user.id && w.status === 'pending');
  if (alreadyPending) {
